...
export class FormComponent implements OnChanges, OnInit {
  ...
  private updateExistingProduct(): void {
    ...
    const productData = {
      ...
      name: this.product.name.trim(), // Cambiado
      description: this.product.description.trim(), // Cambiado
      price: Number(this.product.price), // Cambiado
      category: this.product.category, // Cambiado
      active: this.product.active !== false, // Cambiado
      // Si el backend espera código, usar el existente o generar uno
      productCode: this.product.productCode || `PROD${this.product.id.toString().padStart(3, '0')}` // Cambiado
    };
